      ...
      int fromRank, int toRank ) 
{
   ...
   //
   float *tile = (float *)malloc(sizeof(float)*sendWidth*sendHeight);

   off_t s_offset=srcOffsetColumn, tile_offset=0;

   for (int j=0;j<sendHeight;j++, s_offset+=srcWidth, tile_offset+=sendWidth)
   {
      memcpy((void *)(tile+tile_offset), (void *)(srcBuf+s_offset), sizeof(float)*sendWidth);
   }

   MPI_Send(tile, sendWidth*sendHeight, MPI_FLOAT, toRank, msgTag, MPI_COMM_WORLD);
...
recvStridedBuffer(float *dstBuf, 
   ...
   float *tile = (float *)malloc(sizeof(float)*dstWidth*dstHeight);

   MPI_Recv(tile, dstWidth*dstHeight, MPI_FLOAT, fromRank, msgTag, MPI_COMM_WORLD, &status);

   if (dstWidth*dstHeight != expectedWidth*expectedHeight) {
      // the tile we receive is part of a bigger buffer
      // so we need to adjust it in its right place
      off_t s_offset = 0;
      off_t d_offset = dstOffsetRow * dstWidth + dstOffsetColumn;

      for (int j=0;j<expectedHeight;j++, s_offset+=expectedWidth, d_offset+=dstWidth)
      {
         memcpy((void *)(dstBuf+d_offset), (void *)(tile+s_offset), sizeof(float)*expectedWidth);
      }
   }
   else {
      // we are just receiving a tile and that's what we need
      memcpy((void *)(dstBuf), (void *)(tile), sizeof(float)*expectedWidth*expectedHeight);
   }
...
void
do_sobel_filtering_for_tile(float *in, float *out, int tileWidth, int tileHeight, int tileRowOffset, int tileColOffset)
{
   ...
   for(int i=0; i< tileHeight; i++) {
      for(int j=0; j< tileWidth; j++) {
         out[row_maj_reverse_index(i, j, tileWidth)] = sobel_filtered_pixel(in, i, j, tileWidth, tileHeight, Gx, Gy);
      }
   }
}
   ...
   for (int row=0;row<tileArray.size(); row++)
   {
      ...
      {  
         ...
         //   std::copy(t->inputBuffer.begin(), t->inputBuffer.end(), t->outputBuffer.begin());
            ...
            do_sobel_filtering_for_tile(in, out, t->width, t->height, t->yloc, t->xloc);
         }
      }
   }
}
